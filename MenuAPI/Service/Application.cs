...
namespace RMSAPI.Service
{
    public class Application
    {
        ...
        public Response ViewAllCart(SqlConnection con, int tableValue)
        {
            Response response = new Response();
            SqlDataAdapter da = new SqlDataAdapter("select *  from Table"+ tableValue, con);
            DataTable dt = new DataTable();
        ...
        public Response ViewCartTotal(SqlConnection con, int tableValue)
        {
            con.Open();
            Response response = new Response();
            string query = "SELECT sum(line_Total) from Table"+ tableValue;
            SqlCommand cmd = new SqlCommand(query, con);
        ...
        public Response AddToCart(SqlConnection con, TableCart tableCart,int tableValue)
        {
            con.Open();
            Response response = new Response();
            string query = "INSERT INTO Table"+ tableValue + " VALUES(@FoodID,@FoodName,@Price,@qtyCart)";
            SqlCommand cmd = new SqlCommand(query, con);
        ...
        public Response UpdateCartItem(SqlConnection con, TableCart tableCart, int tableValue)
        {
            Response response = new Response();
            string query = "UPDATE Table"+ tableValue + " SET FoodName = @FoodName, Price = @Price, Quantity=@Quantity WHERE FoodID = @FoodID";
            SqlCommand cmd = new SqlCommand(query, con);
            cmd.Parameters.AddWithValue("@FoodID", tableCart.FoodID);
            cmd.Parameters.AddWithValue("@FoodName", tableCart.FoodName);
            cmd.Parameters.AddWithValue("@Price", tableCart.Price);
        ...
        public Response DeleteCartItem(SqlConnection con, string cartItemID, int tableValue)
        {
            Response response = new Response();
            SqlCommand cmd = new SqlCommand("DELETE from Table"+ tableValue + " WHERE FoodID = '" + cartItemID + "'", con);
            con.Open();
            int i = cmd.ExecuteNonQuery();
            con.Close();
        ...
        public Response PayAndClearCart(SqlConnection con, int tableValue)
        {
            con.Open();
            string sql = "select * from Table"+ tableValue;
            ...
            string sqlquery2 = "DELETE Table"+ tableValue;// Will change this query to send the data to SalesHistoryTable later
